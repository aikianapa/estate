<section class="adv-search-section no-padding">
  ...
  <div id="yandex-map" class="offers-map">
    ...
    <style>ymaps {filter: saturate(0.8);}</style>
  </div>
        ...
        <div class="row">
          ...
          <div class="col-xs-12 col-sm-4">
            <select  data-wb="role=foreach&form=complex&tpl=true" name="complex" class="bootstrap-select" title="Жилой комплекс:" multiple data-actions-box="true"
                ...
                data-watcher='change=#propertiesFilterForm [name=type]'>
                <option value="{{id}}">{{name}}</option>
            </select>
          </div>
          <div class="col-xs-12 col-sm-4">
            <select data-wb="role=tree&item=types&branch=units&parent=false&tpl=true" name="type" class="bootstrap-select" title="Тип:" multiple data-actions-box="true">
                <option value="{{id}}">{{name}}</option>
            </select>
          </div>
        </div>
